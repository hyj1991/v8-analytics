...
function createTree(tree, uidFuncMap, timeout, bailout, only, json) {
    ...
    if (only && bailout) {
        ...
        if (json) {
            jsonList = jsonList.filter(item => item.url && ~item.url.indexOf('/'));
    ...
    if (only && timeout) {
        ...
        onlyWarning = onlyWarning.map((item, index) => index ? `${(index)}. ${item}` : item);
        if (json) {
            jsonList = jsonList.filter(item => item.url && Boolean(~(item.url.indexOf('/'))) && filterFileName(item));
...
function filterFileName(item) {
    return !(['node_modules', 'anonymous'].some(fileName => {
        return Boolean(~(item.url.indexOf(fileName))) || Boolean(~(item.funcName.indexOf(fileName)))
    }));
}
 ...
 */
function getCpuAnalysis(allData, timeout, bailout, only, json) {
