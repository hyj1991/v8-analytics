...
function createTree(tree, uidFuncMap, timeout, bailout, only) {
    let chars = {
        space: '    ',
        pre: warnConsole('│   '),
        first: warnConsole('├── '),
        ...
        tree.forEach(function (node, index) {
            ...
            if (isTimout) {
                ...
                if (only) {
                    ...
                    (!~onlyWarning.indexOf(warnStr)) && onlyWarning.push(warnStr);
                }
            } else {
    ...
    if (only && bailout) {
        onlyWarning.unshift(infoConsole('V8 Engine Optimization Failed List: '));
 ...
 */
function getCpuAnalysis(allData, timeout, bailout, only) {
    ...
    return createTree(finalData, uidFuncMap, timeout, bailout, only);
}
