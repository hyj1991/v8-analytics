...
function fetchHeapUsage(heapData, limit) {
    ...
    const rootIndex = heapData.snapshot.root_index || 0;
    let calculation = heapSnapShotCalculate(heapData);
    let jsHeapSnapShot = calculation.jsHeapSnapShot;
    let heapMap = calculation.heapMap;
    let leakInfo = peakLeakPoint(jsHeapSnapShot, rootIndex, heapMap, limit);
    let leakPoint = leakInfo.leakPoint;
    let statistics = leakInfo.statistics;
    let aggregates = leakInfo.aggregates;
