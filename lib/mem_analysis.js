...
function peakLeakPoint(rootIndex, heapData, heapMap, limit) {
    ...
    return retainedSizeList.reduce((pre, next, index) => {
        ...
        return pre;
    }, {leakPoint: [], length: 0, statistics: jsHeapSnapShot._statistics});
}

//main entry
function fetchHeapUsage(heapData) {
    let {heapMap, heapArray, edgesMap, rootID, rootIndex} = handleHeapData(heapData);
    let {leakPoint, statistics} = peakLeakPoint(rootIndex, heapData, heapMap);

    leakPoint.forEach((item, index) => {
        console.log(`第 ${++index} 个疑似泄漏点：${heapMap[item.index].name}::${item.id},` +
            `类型为：${heapMap[item.index].type}, 其RetainedSize为：${item.size} 字节，` +
            `占据总堆内内存的 ${((item.size / statistics.total) * 100).toFixed(2)}%`
        );
    });
}
...
// const string = fs.readFileSync('../../examples/GC/test/gc--3--1490956634884.heapsnapshot');
