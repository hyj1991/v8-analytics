...
function createTree(tree, uidFuncMap, timeout, bailout, only, json, fileFilter) {
    if (json) {
    ...
    if (only && bailout) {
        ...
        if (json) {
            jsonList = jsonList.filter(item => item.url && fileFilter(item.url, item.funcName));
...
function getCpuAnalysis(allData, timeout, bailout, only, json, fileFilter) {
    ...
    let maps = depthFirst(allData.head);
    let idUidMap = maps.idUidMap;
    let uidFuncMap = maps.uidFuncMap;
    ...
    fileFilter = typeof fileFilter === 'function' && fileFilter || function (url, funcName) { return true }

    return createTree(finalData, uidFuncMap, timeout, bailout, only, json, fileFilter);
}
