 ...
 */
function heapNodeSerialize(jsHeapSnapShot, index, limit) {
...
/**
 * @param {stream} transform @param {number} limit 
 * @return {promise}
 * @description more efficient heap memory analysis
 */
function fetchHeapUsageP(transform, limit) {
    if (!transform instanceof stream.Transform) return;
    ...
    return new Promise((resolve, reject) => {
        ...
        parser.on('data', heapData => {
            ...
            let jsHeapSnapShot = heapSnapShotCalculate(heapData);
            let leakPoint = peakLeakPoint(jsHeapSnapShot, rootIndex, limit);
            //console.log(12333, Object.keys(jsHeapSnapShot));

            resolve({ leakPoint, jsHeapSnapShot, rootIndex });
        });
