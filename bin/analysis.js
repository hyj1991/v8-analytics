...
function createTree(tree, uidFuncMap, timeout) {
    let chars = {
        space: '    ',
        pre: warnConsole('│   '),
        first: warnConsole('├── '),
    ...
    var toString = function (tree, pre) {
        ...
        tree.forEach(function (node, index) {
            ...
            let isWarnig = timeout && (execTime >= timeout) || false;
            let execTimeStr = execTime / 1000 >= 1 && (execTime / 1000).toFixed(2) + 's' || execTime.toFixed(1) + 'ms';
            let percentage = 100;
            if (node.parent) {
...
function getCpuAnalysis(allData, timeout) {
...
function main() {
    ...
    try {
        ...
        console.log(`${infoConsole('File Read Or JSON Parse ')}${errorConsole(e)}`)
    }
}